# Page snapshot

```yaml
- generic [ref=e1]:
  - navigation [ref=e2]:
    - link "QA Playground" [ref=e4] [cursor=pointer]:
      - /url: /
      - heading "QA Playground" [level=1] [ref=e5]
    - link "full courses" [ref=e7] [cursor=pointer]:
      - /url: /#apps
      - img [ref=e8]
      - text: Apps
    - link "Playwright logo View Test Suite" [ref=e19] [cursor=pointer]:
      - /url: https://github.com/marko-simic/qa-playground-tests/blob/main/tests/popup.spec.js
      - img "Playwright logo" [ref=e21]
      - generic [ref=e22]: View Test Suite
  - main [ref=e23]:
    - generic [ref=e24]:
      - paragraph [ref=e25]: Click to open pop-up
      - link "Open" [active] [ref=e26] [cursor=pointer]:
        - /url: "#"
```